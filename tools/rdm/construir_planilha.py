...
def aba_trechos(wb, D, RC):
    ...
    for i, c in enumerate(casas):
        ...
        ws.cell(row=r, column=11,
                value='=IF(N($J%d)>0,$J%d,IF(N($I%d)>0,$I%d,SUMPRODUCT(%s:%s,CADASTRO_CASAS_BOMBA!$H$%d:$H$%d)))'
                      % (r, r, r, r, "$C$%d" % (lin_mapa + i), "$%s$%d" % (gcl(2 + n), lin_mapa + i),
                         RC["ini"], RC["fim"])).style = "num1"
...
def aba_planejamento(wb, D, RP, RR, RPC, RS):
    ...
    est.cabecalho_pagina(ws, "PLANEJAMENTO_SEMANAL  -  motor de calculo (uma linha por SEMANA x PIVO)",
                         ...
                         "horas -> utilizacao. Toda a analise da pasta le esta tabela por SOMASES.", "AJ")
    escrever_tabela(ws, COLS_PS,
                    [8, 12, 12, 10, 13, 13, 10, 10, 12, 16, 22, 13, 10, 12, 13, 17, 8, 13, 11,
                     14, 13, 15, 11, 13, 13, 14, 13, 13, 14, 14, 18, 15, 17, 18, 15])
    ri, rf = RR["ini"], RR["fim"]
    ...
    for p in D["pivos"]:
        for s in D["semanas"]:
            ...
            estilos = ["num0", "data", "data", "rotulo", "texto", "texto", "num2", "num0", "num0",
                       ...
                       "num0", "pct", "texto", "texto", "num1", "num1", "num1", "num1"]
    ...
    ws.auto_filter.ref = "B%d:AJ%d" % (CAB, fim)
    return {"ini": DAT, "fim": fim}
...
def aba_balanco_cultura(wb, D, RPS, RR):
    ...
    est.cabecalho_pagina(ws, "BALANCO DA SAFRA POR CULTURA",
                         "Lamina em mm sempre ponderada pela area: soma(indicador x area) / area plantada da cultura.",
                         "P")
    escrever_tabela(ws, ["Cultura", "Area plantada (ha)", "Ciclos na safra", "ETo durante a ocupacao (mm)",
...
def aba_balanco_pivo(wb, D, RPS, RR, RP):
    ...
    for i, p in enumerate(D["pivos"]):
        ...
        for j, col in enumerate(("AG", "AH", "AI")):
            ws.cell(row=r, column=10 + j,
                    value="=IFERROR(SUMIF(%s,$B%d,%s)/$E%d,0)" % (ps("E"), r, ps(col), r)).style = "num1"
        ws.cell(row=r, column=14, value="=SUMIF(%s,$B%d,%s)" % (ps("E"), r, ps("Y"))).style = "num1"
        ws.cell(row=r, column=15, value="=SUMIF(%s,$B%d,%s)" % (ps("E"), r, ps("Z"))).style = "num0"
        ws.cell(row=r, column=16, value="=IFERROR(INDEX(%s,%s),0)" % (piv("I"), mp)).style = "num1"
        ws.cell(row=r, column=17, value="=SUMIF(%s,$B%d,%s)" % (ps("E"), r, ps("AB"))).style = "num1"
        ws.cell(row=r, column=18, value="=IFERROR(_xlfn.MAXIFS(%s,%s,$B%d),0)" % (ps("AD"), ps("E"), r)
                ).style = "pct"
        ws.cell(row=r, column=19,
                value='=IFERROR(INDEX(%s,MATCH($B%d&"|"&TEXT($R%d,"0.000000"),%s,0)),"-")'
                      % (ps("B"), r, r, ps("AF"))).style = "num0"
        ws.cell(row=r, column=20,
                value='=IF($R%d>LIMITE_CRITICO,"INVIAVEL em 21 h/dia",IF($R%d>LIMITE_ATENCAO,"CRITICO",'
                      'IF($R%d>LIMITE_CONFORTAVEL,"ATENCAO","OK")))' % (r, r, r)).style = "texto"
    fim = DAT + len(D["pivos"]) - 1
    for txt, cor in (("OK", est.VERDE_OK), ("ATENCAO", "FFEB9C"),
                     ("CRITICO", est.LARANJA), ("INVIAVEL", est.VERMELHO_PEND)):
