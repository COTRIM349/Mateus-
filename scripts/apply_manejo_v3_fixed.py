...
chart_path = Path('components/water-balance/CockpitCharts.tsx')
...
# Aba Gráfico é a central operacional.
page = replace_once(
    page,
...
# Substitui, por marcadores estáveis, todo o antigo cockpit + acordeão de detalhes.
start_marker = '      {/* Cockpit do pivô selecionado */}'
end_marker = '\n    </div>\n  );\n}\n\nfunction HeaderChip'
start = page.find(start_marker)
end = page.find(end_marker, start)
if start < 0 or end < 0:
    raise SystemExit(f'area principal não localizada: start={start} end={end}')

main_area = '''      {/* Conteúdo por aba: Gráfico = manejo; Dados = auditoria; Decisão = ação; Lançamento = registro. */}
      {activeTab === "grafico" && (
...
'''
page = page[:start] + main_area + page[end:]

# Entradas e Consumo recebe a fase calculada pelo motor.
page = replace_once(page, '    kc: r.kc,\n    isForecast: false,', '    kc: r.kc,\n    phase: r.phase,\n    isForecast: false,', 'phase hist')
page = replace_once(page, '      kc: p.kc,\n      isForecast: true,', '      kc: p.kc,\n      phase: p.phase,\n      isForecast: true,', 'phase forecast')

# Cockpit recebe o perfil físico do pivô.
page = replace_once(
    page,
    '  projection,\n  identity,\n  sensoryByDate,',
    '  projection,\n  soilLayers,\n  identity,\n  sensoryByDate,',
...
# Perfil radicular real por camada + CTA completa.
old_kpis = '''  const bottomKpis = [
...
new_kpis = '''  const rootCm = Math.max(last.rootDepth * 100, 0);
...
page = replace_once(page, old_kpis, new_kpis, 'top KPI/profile metrics')

# Topo técnico do pivô.
page = replace_once(
    page,
...
# Remove os 6 cards duplicados do cabeçalho do pivô, usando marcadores exatos.
metrics_start = page.find('        <div className="mt-3.5 grid grid-cols-2 gap-px overflow-hidden rounded-xl bg-gray-100 sm:grid-cols-3 lg:grid-cols-6 dark:bg-white/[0.06]">', page.find('function Cockpit('))
metrics_end_marker = '\n        </div>\n      </Card>\n\n      {/* Gráficos + coluna de decisão */}'
metrics_end = page.find(metrics_end_marker, metrics_start)
if metrics_start < 0 or metrics_end < 0:
    raise SystemExit(f'cards duplicados não localizados: {metrics_start=} {metrics_end=}')
page = page[:metrics_start] + '      </Card>\n\n      {/* Gráficos + coluna de decisão */}' + page[metrics_end + len(metrics_end_marker):]

# Gráficos protagonistas.
page = replace_once(page, 'className="h-[220px] w-full"><EntradasConsumoChart', 'className="h-[270px] w-full"><EntradasConsumoChart', 'entries height')
page = replace_once(page, 'className="h-[260px] w-full"><ReservatorioChart', 'className="h-[330px] w-full"><ReservatorioChart', 'reservoir height')

# Perfil explorado pela raiz no lado direito.
profile_card = '''
          <Card className="p-4">
...
'''
page = replace_once(page, '          {/* Nota de umidade de campo + Alertas */}', profile_card + '          {/* Nota de umidade de campo + Alertas */}', 'root profile card')

# Remove KPI agronômico duplicado do rodapé.
footer_start = page.find('      {/* KPIs agronômicos (rodapé) */}', page.find('function Cockpit('))
footer_end_marker = '\n    </div>\n  );\n}\n\nfunction Legend'
footer_end = page.find(footer_end_marker, footer_start)
if footer_start < 0 or footer_end < 0:
    raise SystemExit(f'rodapé KPI não localizado: {footer_start=} {footer_end=}')
page = page[:footer_start] + page[footer_end:]

# Gráfico de entrada: faixa fenológica por data.
chart = replace_once(chart, '  kc?: number | null;\n  isForecast: boolean;', '  kc?: number | null;\n  phase?: string | null;\n  isForecast: boolean;', 'chart phase type')
chart = replace_once(chart, '  const padB = 28;', '  const padB = 48;', 'chart bottom padding')
chart = replace_once(
    chart,
...
print('Manejo V3 patch corrigido aplicado.')
