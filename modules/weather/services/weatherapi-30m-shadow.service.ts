...
import { fetchWeatherApiHourlyRaw } from "@/modules/weather/providers/weatherApiHourly";
import { normalizeWeatherApiHourlyTo30Min } from "@/modules/weather/normalizers/normalizeWeatherApi30Min";
...
export interface WeatherApi30MinSyncResult {
  stationId: string;
  rawPayloadId: string;
  fetchedAt: string;
  rowsInserted: number;
...
export async function syncWeatherApi30MinShadow(
  supabase: SupabaseClient,
  virtualStationId: string,
): Promise<WeatherApi30MinSyncResult> {
  ...
  if (!station.shadow_mode) throw new Error("CLIMA 4 so pode executar em shadow_mode");
  ...
  const { data: providerConfig, error: providerError } = await supabase
    ...
    .eq("provider", "weatherapi")
    .single();

  if (providerError || !providerConfig?.enabled) {
    throw new Error("WeatherAPI nao esta habilitada para esta estacao virtual");
  }

  const location = toLocation(station);
  const result = await fetchWeatherApiHourlyRaw(location);
  const normalized = normalizeWeatherApiHourlyTo30Min({
  ...
  const { data: rawRow, error: rawError } = await supabase
    ...
    .insert({
      ...
      provider: "weatherapi",
      data_type: "forecast",
      model_name: "weatherapi_forecast",
      ...
      response_latitude: result.payload.location?.lat ?? null,
      response_longitude: result.payload.location?.lon ?? null,
      response_elevation_m: null,
  ...
  if (rawError || !rawRow) {
    throw new Error(rawError?.message ?? "Falha ao persistir payload bruto WeatherAPI");
  }
