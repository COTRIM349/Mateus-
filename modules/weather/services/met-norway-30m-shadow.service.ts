...
import { fetchMetNorwayHourlyRaw } from "@/modules/weather/providers/metNorwayHourly";
import { normalizeMetNorwayHourlyTo30Min } from "@/modules/weather/normalizers/normalizeMetNorway30Min";
...
export interface MetNorway30MinSyncResult {
  stationId: string;
  rawPayloadId: string;
  fetchedAt: string;
  rowsInserted: number;
...
export async function syncMetNorway30MinShadow(
  supabase: SupabaseClient,
  virtualStationId: string,
): Promise<MetNorway30MinSyncResult> {
  ...
  if (!station.shadow_mode) throw new Error("CLIMA 5 so pode executar em shadow_mode");
  ...
  if (providerError || !providerConfig?.enabled) {
    throw new Error("MET Norway nao esta habilitado para esta estacao virtual");
  }

  const location = toLocation(station);
  const result = await fetchMetNorwayHourlyRaw(location);
  const normalized = normalizeMetNorwayHourlyTo30Min({
  ...
  const coordinates = result.payload.geometry?.coordinates;
  const issuedAt = result.payload.properties?.meta?.updated_at ?? null;
  ...
  const { data: rawRow, error: rawError } = await supabase
    ...
    .insert({
      ...
      provider: "met_norway",
      data_type: "forecast",
      model_name: "met_norway_locationforecast_compact",
      ...
      response_latitude: coordinates?.[1] ?? null,
      response_longitude: coordinates?.[0] ?? null,
      response_elevation_m: coordinates?.[2] ?? null,
  ...
  if (rawError || !rawRow) {
    throw new Error(rawError?.message ?? "Falha ao persistir payload bruto MET Norway");
  }
