// ============================================================================
// MOTOR HÍDRICO V3 — Kc DUAL FAO-56
// ============================================================================
// ETc = ETo × (Ks × Kcb × Kl + Ke)
// Kcb = coeficiente basal interpolado por fase; Ke = evaporação do solo;
// Ks atua somente sobre a transpiração. O balanço superficial (De) é contínuo.
// ============================================================================
...
import { computeSurfaceEvaporationDay } from "./dual-crop-coefficient";

export type HydricStatus = "verde" | "amarelo" | "vermelho" | "cinza";

export const HYDRIC_ENGINE_VERSION = "hydric-v3-dual" as const;
...
export interface EngineCulture {
  ...
  ky?: number | null;
  coefficient_method?: string | null;
...
export type DualCulturePhase = CulturePhase & {
...
export interface PivotEngineInput {
  ...
  initialSurfaceDepletionMm?: number | null;
}
...
export interface BalanceDay {
  ...
  dae: number;
  phase: string;
  /** Compatibilidade: Kc efetivo potencial = Kcb ajustado × Kl + Ke. */
  kc: number;
  /** Compatibilidade: Kc efetivo real = Ks × Kcb ajustado × Kl + Ke. */
  kcAdjusted: number;
  coefficientMethod: "dual_fao56";
  kcbReference: number;
  kcbAdjusted: number;
  kcbSource: string;
  climateAdjustmentSource: string;
  ke: number;
  kcMax: number;
  kr: number;
  ...
  surfaceDepletionStartMm: number;
  surfaceDepletionMm: number;
  tewMm: number;
  rewMm: number;
...
export function hasCompleteWeatherSeries(weatherByDate: Record<string, EngineWeatherDay>, start: string, end: string): boolean {
  return dateRange(start, end).every((date) => {
    ...
    return Boolean(w && Number.isFinite(w.et0) && w.et0 >= 0 && Number.isFinite(w.precipitation) && w.precipitation >= 0);
  });
}

function resolveApplicationEfficiency(assignment: EngineAssignment, pivot: EnginePivot): number {
  if (assignment.parameter_mode === "personalizado" && assignment.irrigation_efficiency != null) return assignment.irrigation_efficiency;
...
function buildRecommendation(deficit: number, afd: number, status: HydricStatus, efficiency: number, area: number, flowRate: number, deficitIrrigation = false) {
  ...
  return { shouldIrrigate: true, netDepth, grossDepth, volume, time, reason: `Déficit ${deficit.toFixed(1)} mm atingiu a AFD (${afd.toFixed(1)} mm). Recomendar ${grossDepth.toFixed(1)} mm brutos com Ea ${(efficiency * 100).toFixed(0)}%.` };
}
...
export function computePivotBalanceSeries(input: PivotEngineInput): BalanceDay[] {
  ...
  let previousSurfaceDepletion: number | null = input.initialSurfaceDepletionMm ?? null;

  for (const date of dates) {
    const ms = new Date(`${date}T00:00:00Z`).getTime();
    const dae = Math.max(0, Math.floor((ms - daeRefMs) / 86400000));
    const phaseId = identifyPhase(phases, dae);
    if (!phaseId) return [];
    const phase = phaseId.phase as DualCulturePhase;
    if (phase.kcb_start == null || phase.kcb_end == null) return [];

    const rootDepth = computeRootDepth({ phases, dae, cultureRootDepth: culture.root_depth, initialRootDepth: custom ? assignment.initial_root_depth : null, maxRootDepth: custom ? assignment.max_root_depth : null });
    const phaseName = phase.name;
    const pFactor = resolveDepletionFactor(assignment, phase.depletion_factor, culture.depletion_factor);
    const adt = soil.layers && soil.layers.length > 0 ? calculateADTFromLayers(soil.layers, rootDepth) : calculateADT(soil.field_capacity, soil.wilting_point, rootDepth, soil.effective_depth);
    if (adt <= 0) return [];
    ...
    const kl = resolveManejoKl({ parcelOverride: assignment.kl_override, phaseKl: phase.kl, cultureKl: culture.kl });

    let armStart: number;
    if (previousStorage != null) {
      armStart = scaleArmToNewCad(previousStorage, previousCad ?? adt, adt);
    } else {
      const initial = initialArmFromMoisture({ cadMm: adt, thetaCc: fieldCapacity, thetaPmp: wiltingPoint, bulkDensity: soil.bulk_density, moisturePct: assignment.initial_soil_moisture_pct, unit: assignment.initial_moisture_unit, isFieldCapacity: assignment.initial_moisture_is_cc });
    ...
    const ksConfigured = resolveKsFunctionName(assignment.ks_function_override, phase.ks_function, culture.ks_function);
    ...
    const surface = computeSurfaceEvaporationDay({
      phase,
      phaseProgress: phaseId.progress,
      et0Mm: weather.et0,
      precipitationMm: weather.precipitation,
      effectiveIrrigationMm: effectiveIrrigation,
      soil,
      previousDeMm: previousSurfaceDepletion,
      weather: { wind_speed_2m: weather.wind_speed_2m, rh_min: weather.rh_min },
      wettedFraction: 1,
    });
    if (!surface) return [];

    const transpirationPotential = Math.max(weather.et0 * surface.kcbAdjusted * kl, 0);
    const etcPotential = roundTo(transpirationPotential + surface.soilEvaporationMm, 2);
    const etc = roundTo(Math.max(weather.et0 * (ks * surface.kcbAdjusted * kl + surface.ke), 0), 2);
    const kc = roundTo(surface.kcbAdjusted * kl + surface.ke, 3);
    const kcAdjusted = roundTo(ks * surface.kcbAdjusted * kl + surface.ke, 3);
    const ky = resolvePhaseKy(phase.ky, culture.ky);
    const yieldRisk = yieldRiskFraction(ky, ks);

    const step = applyDailySoilBalance({ armStart, cad: adt, precipitation: weather.precipitation, effectiveIrrigation, etc });
    ...
    rows.push({
      date, dae, phase: phaseName, kc, kcAdjusted, coefficientMethod: "dual_fao56",
      kcbReference: surface.kcbReference,
      kcbAdjusted: surface.kcbAdjusted,
      kcbSource: phase.kcb_reference_source ?? culture.kcb_reference_source ?? "FAO-56",
      climateAdjustmentSource: surface.climateAdjustmentSource,
      ke: surface.ke, kcMax: surface.kcMax, kr: surface.kr,
      canopyCoverFraction: surface.canopyCoverFraction,
      wettedFraction: surface.wettedFraction,
      exposedWettedFraction: surface.exposedWettedFraction,
      surfaceDepletionStartMm: surface.deStartMm,
      surfaceDepletionMm: surface.deEndMm,
      tewMm: surface.tewMm, rewMm: surface.rewMm,
      evaporationLayerDepthM: surface.evaporationLayerDepthM,
      soilEvaporationMm: surface.soilEvaporationMm,
      ks, kl, et0: roundTo(weather.et0, 2), etcPotential, etc,
      etcFormula: `ETc = ETo × (Ks × Kcb × Kl + Ke) = ${roundTo(weather.et0, 2)} × (${ks} × ${surface.kcbAdjusted} × ${kl} + ${surface.ke}) = ${etc} mm`,
      ky, yieldRisk,
      precipitation: roundTo(weather.precipitation, 2), effectivePrecipitation: step.pe, peFormula: step.peFormula,
      irrigation: roundTo(irrigation, 2), effectiveIrrigation,
      rootDepth: roundTo(rootDepth, 3), adt, afd, storage, surplus: step.surplus, deficit, depletion,
      fieldCapacity, wiltingPoint, safetyMoistureMm: safetyMm, moisturePctCc, safetyPctCc,
      balanceFormula: step.balanceFormula, status, mapStatus,
      shouldIrrigate: rec.shouldIrrigate, recommendedNetDepth: rec.netDepth, recommendedGrossDepth: rec.grossDepth,
      recommendedVolume: rec.volume, estimatedIrrigationTime: rec.time, recommendationReason: rec.reason,
    });
    previousStorage = storage;
    previousCad = adt;
    previousSurfaceDepletion = surface.deEndMm;
  }
  return rows;
}

export interface PivotHydricState {
  pivotId: string; pivotName: string; cultureName: string; varietyName: string | null; seasonName: string | null;
  area: number; latitude: number; longitude: number; parcelId: string | null; plantingDate: string | null;
  soilName: string | null; radiusMeters: number | null; sheetIncomplete: boolean; startAngleDeg: number | null;
  endAngleDeg: number | null; parcelName: string | null; current: BalanceDay | null; history: BalanceDay[];
}
...
export interface PivotIdentity {
  ...
  area: number; latitude: number; longitude: number; parcelId: string | null; plantingDate?: string | null;
  soilName?: string | null; radiusMeters?: number | null; sheetIncomplete?: boolean; startAngleDeg?: number | null;
  endAngleDeg?: number | null; parcelName?: string | null;
}
...
export function computePivotCurrentState(identity: PivotIdentity, input: PivotEngineInput): PivotHydricState {
  ...
  return { ...identity, plantingDate: identity.plantingDate ?? null, soilName: identity.soilName ?? null, radiusMeters: identity.radiusMeters ?? null, sheetIncomplete: identity.sheetIncomplete ?? false, startAngleDeg: identity.startAngleDeg ?? null, endAngleDeg: identity.endAngleDeg ?? null, parcelName: identity.parcelName ?? null, current: history.length > 0 ? history[history.length - 1] : null, history };
}

export interface FarmHydricSummary {
  totalPivots: number; needIrrigationToday: number; attention: number; adequate: number; noData: number;
  totalIrrigatedArea: number; areaInDeficit: number; avgRecommendedDepth: number; avgDeficit: number;
  totalRecommendedVolume: number; ranking: PivotHydricState[]; priorityList: PivotHydricState[];
}
...
export function computeFarmHydricState(states: PivotHydricState[]): FarmHydricSummary {
  ...
  return { totalPivots: new Set(states.map((s) => s.pivotId)).size, needIrrigationToday: needing.length, attention: attention.length, adequate: adequate.length, noData: noData.length, totalIrrigatedArea: roundTo(totalIrrigatedArea, 2), areaInDeficit: roundTo(areaInDeficit, 2), avgRecommendedDepth, avgDeficit, totalRecommendedVolume, ranking, priorityList };
}
