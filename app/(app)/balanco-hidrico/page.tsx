        ...
        <>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-white/[0.08] dark:bg-graphite-800 lg:col-span-2">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h2 className="font-semibold text-gray-900 dark:text-white">Recomendação operacional</h2>
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-white/[0.08] dark:bg-graphite-800">
              ...
              <dl className="mt-4 space-y-3 text-sm">
                <Row label="Pivô" value={selected.pivotName} />
                <Row label="Cultura" value={selected.cultureName} />
                <Row label="Solo" value={selected.soilName ?? "—"} />
                <Row label="Data do cálculo" value={datePtBr(current.date)} />
                <Row label="Fórmula ETc" value={current.etcFormula} />
                <Row label="Balanço" value={current.balanceFormula} />
              </dl>
            </div>
          </div>
          ...
          <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-white/[0.08] dark:bg-graphite-800">
            <div className="border-b border-gray-200 px-5 py-4 dark:border-white/[0.08]">
              <h2 className="font-semibold text-gray-900 dark:text-white">Histórico recalculado V2</h2>
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-gray-50 text-left text-xs uppercase tracking-wide text-gray-500 dark:bg-graphite-900 dark:text-gray-400">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right">DAE</th>
                    <th className="px-4 py-3">Fase</th>
                    <th className="px-4 py-3 text-right">ETo</th>
                    <th className="px-4 py-3 text-right">ETc</th>
                    <th className="px-4 py-3 text-right">Chuva</th>
                    <th className="px-4 py-3 text-right">Irrig.</th>
                    <th className="px-4 py-3 text-right">ARM</th>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-white/[0.06]">
                  {[...history].reverse().map((day) => (
                    <tr key={day.date}>
                      <td className="px-4 py-3 text-gray-800 dark:text-gray-200">{datePtBr(day.date)}</td>
                      <td className="px-4 py-3 text-right">{day.dae}</td>
                      <td className="px-4 py-3">{day.phase}</td>
                      <td className="px-4 py-3 text-right">{number(day.et0, 2)}</td>
                      <td className="px-4 py-3 text-right">{number(day.etc, 2)}</td>
                      <td className="px-4 py-3 text-right">{number(day.precipitation)}</td>
                      <td className="px-4 py-3 text-right">{number(day.irrigation)}</td>
                      <td className="px-4 py-3 text-right font-medium">{number(day.storage)}</td>
                      <td className="px-4 py-3 text-right">{number(day.deficit)}</td>
                      <td className="px-4 py-3">{STATUS_LABELS[day.mapStatus] ?? day.mapStatus}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function MetricCard({ label, value, detail }: { label: string; value: string; detail: string }) {
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-white/[0.08] dark:bg-graphite-800">
