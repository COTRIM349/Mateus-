...
export async function POST(request: Request) {
  ...
  try {
    const result = await syncWeatherApi30MinShadow(supabase, virtualStationId);
    return NextResponse.json({ ok: true, result });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Falha na sincronizacao WeatherAPI";
